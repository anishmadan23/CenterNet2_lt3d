...
class CenterNet(nn.Module):
    ...
    def update_gt_with_pseudolabels(self, targets, file_names, pseudolabel_path, confidence_thresh=None, zs_preds=False):
        # import ipdb; ipdb.set_trace()
        if not zs_preds:     # extra GT annotations
            all_gt_annos = pseudolabel_path
            device = targets[0]._fields['gt_boxes'].device
        ...
        for idx, target_info in enumerate(targets):
            ...
            if zs_preds:
                ...
                selected_file_name = 'samples/'+img_path.split('samples/')[1]
                my_gt_annos = all_gt_annos[selected_file_name]
                all_gt_boxes = [[anno['bbox'][0], anno['bbox'][1], anno['bbox'][0]+ anno['bbox'][2], anno['bbox'][1]+anno['bbox'][3]] for anno in my_gt_annos]
                all_gt_classes = [anno['category_id'] for anno in my_gt_annos]

                all_gt_boxes_tsor = Boxes(torch.Tensor(all_gt_boxes).to(device))
                all_gt_classes_tsor = torch.Tensor(all_gt_classes).to(device).long()

                instance_gt_anno = Instances(target_info.image_size)
                instance_gt_anno.gt_boxes = all_gt_boxes_tsor
                instance_gt_anno.gt_classes = all_gt_classes_tsor 

                targets[idx] = instance_gt_anno
                # targets[idx]._fields['gt_boxes'].tensor = all_gt_classes_tsor
    ...
    def forward(self, images, features_dict, gt_instances, file_names=None, negloss_boxes_path=None, zs_conf_thresh=None, zs_negboxes=None):
        ...
        if not self.training:
            ...
            if negloss_boxes_path is not None:
                ...
                augmented_gt_instances = self.update_gt_with_pseudolabels([x.copy_instance() for x in gt_instances], file_names, negloss_boxes_path, confidence_thresh=zs_conf_thresh, zs_preds=zs_negboxes)
                # augmented_pos_inds, _, _, _ = self._get_augmented_hm(grids, shapes_per_level, negloss_boxes_path)

                augmented_pos_inds, _, _, augmented_flattened_hms = \
                    self._get_ground_truth(
    ...
    def losses(
        ...
        if self.with_agn_hm:
            if augmented_flattened_hms is not None:
                cat_agn_heatmap = augmented_flattened_hms.max(dim=1)[0] # M
            else:
